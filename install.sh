...
main() {
    ...
    INSTALL_BIN_FILENAME=andresguard
    ...
    INSTALL_LIB_FILENAME="AndResGuard-cli-$VERSION.jar"
    INSTALL_LIB_FILEPATH="$INSTALL_LIB_DIR/$INSTALL_LIB_FILENAME"
    ...
    for item in "$INSTALL_BIN_FILEPATH" "$INSTALL_LIB_FILEPATH" "$INSTALL_ZSH_COMPLETION_FILEPATH"
    do
        if [ -e "$item" ] ; then
            die "$item is already exist."
        fi
    done

    on_exit() {
        [ "$INSTALL_SUCCESS" = true ] && return 0
    ...
    if [ ! -d "$INSTALL_LIB_DIR" ] ; then
        run install -d "$INSTALL_LIB_DIR"
    fi

    if [ -w "$INSTALL_LIB_DIR" ] ; then
        run      install -m 444 "lib/$INSTALL_LIB_FILENAME" "$INSTALL_LIB_FILEPATH"
    else
        run sudo install -m 444 "lib/$INSTALL_LIB_FILENAME" "$INSTALL_LIB_FILEPATH"
    fi
